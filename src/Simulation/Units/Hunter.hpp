...
namespace sw::units
{
    ...
    private:
		...
		attributes::BaseAttribute agility;
		attributes::BaseAttribute strength;

    public:
        Hunter(uint32_t id, int32_t x, int32_t y, int32_t hp, 
               int32_t agility, int32_t strength, int32_t range)
            : Unit(id, x, y, hp),
				combat(attributes::CombatAttributes::ranged(range, range)),
				agility(1),
				strength(1)
        {
		...
		{
			return range > 1 ? agility.GetValue() : strength.GetValue();
		}
        ...
        void act(Map& map, EventLog& log, uint32_t tick, std::vector<Unit*>& scratchBuffer) override
        {
            ...
            scratchBuffer.clear();
			map.getUnitsInRange(id, transform.x, transform.y, 1, 1, scratchBuffer);
            ...
            if (!scratchBuffer.empty())
            {
				...
				auto damage = getDamage(combat.maxRange.GetValue());
				Unit* target = scratchBuffer[dis(gen)];
				target->takeDamage(damage);
